				...
				<% with $Venue %>
					...
					<div class="map-header">
						...
						<div class="map-container">
							<div id="mini-map" style="width: 100%; height: 100%" data-link="$Link" 
							<% if $Latitude && $Longitude %> 
							data-lat="$Latitude" data-lng="$Longitude" 
							<% else %> 
							data-address="$Address" <% end_if %> data-title="$Title.LimitCharacters(20)"></div>
						</div>
					<% end_if %>
				<% end_with %>
				
				<%--this needs to go in map header somewhere--%>
				<p class="venue-nav">
					<% with $Venue %>
						<% if $Address %>

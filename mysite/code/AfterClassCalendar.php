...
class AfterClassCalendar_Controller extends Calendar_Controller {
	...
	function UpcomingDatesAndRanges(){
 	...
 	public function Feed(){
 		...
 		switch($feedType){
 			case "json":
 				return $this->getJsonFeed($this->UpcomingDatesAndRanges());
 				break;
 			case "rss":
 	...
 	public function getJsonFeed($eventDates){
 		if(!isset($eventDates)){
 			$eventDates = $this->UpcomingEvents();
 		}
 		$data = array();

 		foreach($eventDates as $eventDateNum => $eventDate){

 			/* Get Dates in  an array for later */
 			/*$datesArray = array();
 			$dates = $event->DateTimes();*/
 			$event = AfterClassEvent::get()->filter(array("ID" => $eventDate->Event()->ID))->First();

 			if($event){


			$datesArray[0]["start_date"] = $eventDate->StartDate;
			$datesArray[0]["start_time"] = $eventDate->StartTime;
			$datesArray[0]["end_date"] = $eventDate->EndDate;
			$datesArray[0]["end_time"] = $eventDate->EndTime;
			$datesArray[0]["all_day"] = $eventDate->AllDay;
 			...
 			$data["events"][$eventDateNum]["id"] = $event->ID;
 			$data["events"][$eventDateNum]["name"] = $event->Title;
 			$data["events"][$eventDateNum]["link"] = $event->AbsoluteLink();
 			$data["events"][$eventDateNum]["more_info_link"] = $event->MoreInfoLink;
 			if($event->Image()){
 				$data["events"][$eventDateNum]["image"] = $event->Image()->CroppedImage(730, 462) ? $event->Image()->CroppedImage(730, 462)->getAbsoluteURL(): $event->Image()->getAbsoluteURL();
 				//$data["events"][$eventNum]["small_image"] = $event->Image()->SmallImage() ? $event->Image->SmallImage()->getAbsoluteURL(): null;
 			}
 			$data["events"][$eventDateNum]["cancel_note"] = $event->CancelReason;
 			$data["events"][$eventDateNum]["date"] = $datesArray;
 			$data["events"][$eventDateNum]["price"] = $event->Cost;
 			$data["events"][$eventDateNum]["location"] = $event->Location;
 			$data["events"][$eventDateNum]["venues"] = $venuesArray;
 			$data["events"][$eventDateNum]["sponsors"] = $sponsorsArray;
 			$data["events"][$eventDateNum]["event_types"] = $eventTypesArray;
 			unset($datesArray);
 		}
 	}
 		echo json_encode($data);
